...
// Função auxiliar para manter o heap
void heapify(Show* array[], int n, int i) {
    int maior = i; // Inicializa maior como raiz
    int esq = 2 * i + 1;
    int dir = 2 * i + 2;

    // Compara o atributo director e title
    if (esq < n && (strcasecmp(array[esq]->director, array[maior]->director) > 0 ||
        (strcasecmp(array[esq]->director, array[maior]->director) == 0 &&
         strcasecmp(array[esq]->title, array[maior]->title) > 0))) {
        maior = esq;
    }

    if (dir < n && (strcasecmp(array[dir]->director, array[maior]->director) > 0 ||
        (strcasecmp(array[dir]->director, array[maior]->director) == 0 &&
         strcasecmp(array[dir]->title, array[maior]->title) > 0))) {
        maior = dir;
    }
    ...
    if (maior != i) {
        ...
        // Recursivamente aplica heapify
        heapify(array, n, maior);
    }
}

// Algoritmo de ordenação por heapsort
