...
void selecaoRecursiva(Show* array[], int i, int n) {
    if (i >= n - 1) return;

    int menor = i;
    for (int j = i + 1; j < n; j++) {
        total_comparacoes++;
        if (strcmp(array[j]->title, array[menor]->title) < 0) {
            menor = j;
        }
    }
...
int main() {
    ...
    while (fgets(entrada, sizeof(entrada), stdin)) {
        ...
        for (int i = 0; i < show_count; i++) {
            if (strcmp(shows[i].show_id, entrada) == 0) {
                selecionados[qtdSelecionados++] = &shows[i];
                break;
            }
        }
    }
    ...
    selecaoRecursiva(selecionados, 0, qtdSelecionados);
    clock_t fim = clock();

    for (int i = 0; i < qtdSelecionados; i++) {
        print_show(selecionados[i]);
    }
    ...
    if (log) {
        fprintf(log, "843309\t%d\t%d\t%lf\n", total_comparacoes, total_movimentacoes, tempo);
        fclose(log);
    }
